				      ...
				      PangoGlyphString *glyphs)
{
  ...
  int *byte_offset = alloca (sizeof (int) * n_chars);
...
//      g_assert (glyphs->log_clusters[glyphix] < n_chars);
      if (glyphs->log_clusters[glyphix] < n_chars)
		 ...
		 PangoGlyphString *glyphs)
{
  ...
  if (retval)
    old_font = SelectObject (g_hdc, hfont);
  ...
  if (retval)
    {
      ...
      retval = itemize_shape_and_place (font, g_hdc, wtext, wlen, analysis, glyphs, script_cache);
...
static gboolean
text_is_simple (const char *wtext,
		gint        wlen)
{
  return ((*script_is_complex) (wtext, wlen, SIC_COMPLEX) == S_FALSE);
}
  ...
  if (have_uniscribe)
    {
      ...
      while (i < length)
      {
         ...
         {
	   glong wlen;
	   char *wtext = g_utf8_to_utf16 (text, length, NULL, &wlen, NULL);
           if (wtext)
           {
              if (!text_is_simple (wtext, wlen))
	      {
		uniscribe_shape (font, wtext, wlen, text, length, analysis, glyphs);
                g_free (wtext);
		return;
	      }
              g_free (wtext);
