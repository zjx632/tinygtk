...
init_reset_sockets (GIOWin32Channel *channel)
{
  ...
  bind (channel->reset_send, (struct sockaddr *)&local, sizeof (local));
  ...
  bind (channel->reset_recv, (struct sockaddr *)&local2, sizeof (local));
  
  len = sizeof (local2);
  getsockname (channel->reset_recv, (struct sockaddr *)&local2, &len);
