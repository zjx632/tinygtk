...
{
  int vs = GetSystemMetrics(SM_CXVSCROLL);
  if (!uxtheme_dll)
    vs -= 2;
...
if(!uxtheme_dll)
  gtk_paint_box (widget->style,
		 widget->window,
		 GTK_STATE_NORMAL, shadow_type, //state_type, shadow_type,
		 &intersection, widget,
		  ...
		  GdkEventExpose *event)
{
  ...
  if(range->layout->grab_button != 0 && range->layout->mouse_location == MOUSE_SLIDER)
