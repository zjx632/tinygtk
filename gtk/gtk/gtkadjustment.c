			     ...
			     gdouble        delta)
{
	...
	goal = adjustment->value + delta;

  if (goal > (adjustment->upper - adjustment->page_size))
	goal = (adjustment->upper - adjustment->page_size);

  if (goal < adjustment->lower)
	goal = adjustment->lower;

	gtk_adjustment_set_value (adjustment, goal);
}
...
gtk_adjustment_wheel_up (GtkAdjustment *adjustment)
{
  ...
  SystemParametersInfo (0x0068, sizeof (delta), &delta, 0);
  if (delta > adjustment->page_size)
	delta = adjustment->page_size;

  sdelta = ((double)delta * adjustment->step_increment);

  gtk_adjustment_change_value (adjustment, -sdelta);
}
