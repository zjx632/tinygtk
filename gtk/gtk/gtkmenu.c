...
gtk_menu_new (void)
{
  GtkWidget *wid = g_object_new (GTK_TYPE_MENU, NULL);
		...
		GdkEventExpose *event)
{
  ...
  if (event->window == widget->window)
    {
	/* white */
        gdk_draw_rectangle (widget->window, widget->style->base_gc[0], TRUE,
			1, 1, width-2, height-2);

	/* dark gray */
        gdk_draw_rectangle (widget->window, widget->style->dark_gc[0], FALSE,
...
gtk_menu_position (GtkMenu *menu)
{
  ...
  gtk_window_move (GTK_WINDOW (menu->toplevel), 
		   x, y);
...
gtk_menu_remove_scroll_timeout (GtkMenu *menu)
{
  if (menu->timeout_id)
    {
      g_timeout_remove (menu->timeout_id);
      menu->timeout_id = 0;
    }
}
			   ...
			   GtkScrollType  type)
{
  ...
  switch (type)
	...
	menu_shell->ignore_enter = TRUE;
	old_upper_arrow_visible = menu->upper_arrow_visible;
	...
	if (menu_shell->active_menu_item)
	  {
	    GtkWidget *new_child;
	    gboolean new_upper_arrow_visible = menu->upper_arrow_visible;
