...
/* --- variables --- */
static GdkColor gtk_default_normal_fg;
static GdkColor gtk_default_active_fg;
static GdkColor gtk_default_prelight_fg;
static GdkColor gtk_default_selected_fg;
static GdkColor gtk_default_insensitive_fg;

static GdkColor gtk_default_normal_bg;
static GdkColor gtk_default_active_bg;
static GdkColor gtk_default_prelight_bg;
static GdkColor gtk_default_selected_bg;
static GdkColor gtk_default_insensitive_bg;

static GdkColor gtk_default_normal_base;
static GdkColor gtk_default_active_base;
static GdkColor gtk_default_prelight_base;
static GdkColor gtk_default_selected_base;
static GdkColor gtk_default_insensitive_base;

static GdkColor gtk_default_normal_text;
static GdkColor gtk_default_active_text;
static GdkColor gtk_default_prelight_text;
static GdkColor gtk_default_selected_text;
static GdkColor gtk_default_insensitive_text;

static GdkColor gtk_default_light;
static GdkColor gtk_default_dark;
...
HINSTANCE uxtheme_dll = 0;

#define XP_BUTTON 0
#define XP_VSCROLLBAR 1
#define XP_HSCROLLBAR 2
#define XP_ARROWUP 3
#define XP_ARROWDOWN 4
#define XP_ARROWLEFT 5
#define XP_ARROWRIGHT 6
#define XP_VGRIP 7
#define XP_HGRIP 8
#define XP_VTROUGH 9
#define XP_HTROUGH 10
#define XP_CHECKBOX 11
#define XP_CHECKBOXP 12
#define XP_EDITTEXT 13
#define XP_LISTHEAD 14
#define XP_TAB 15
#define XP_COMBOBUTTON 16
#define XP_BODY 17
#define XP_RADIO 18
#define XP_RADIOP 19
#define XP_NUM 20

static const short xp_bg_types[XP_NUM]=
{
	BP_PUSHBUTTON,//0
	SBP_THUMBBTNVERT,//1
	SBP_THUMBBTNHORZ,//2
	...
	SBP_GRIPPERVERT,//7
	SBP_GRIPPERHORZ,//8
	SBP_LOWERTRACKVERT,//9
	SBP_LOWERTRACKHORZ,//19
	BP_CHECKBOX,//11
	BP_CHECKBOX,//12
	EP_EDITTEXT,//13
	HP_HEADERITEM,//14
	TABP_TABITEM,//15
	CP_DROPDOWNBUTTON,//16
	TABP_BODY,//17
	BP_RADIOBUTTON,//18
	BP_RADIOBUTTON,//19
};
...
#define THEMES_MAX 6
static HTHEME open_themes[THEMES_MAX];



/* functions */
static HTHEME (FAR PASCAL *xp_OpenThemeData) (HWND, LPCWSTR);
static int (FAR PASCAL *xp_DrawThemeBackground) (HTHEME hTheme, HDC hdc, int iPartId,
	int iStateId, const RECT *pRect, const RECT *pClipRect);
static void (FAR PASCAL *xp_CloseThemeData) (HTHEME theme);
static void (FAR PASCAL *xp_EnableThemeDialogTexture) (HWND hwnd, DWORD dwFlags);

void xp_open ();
static void xp_close ();
		    ...
		    gpointer  data)
{
  ...
  switch (msg->message)
	...
	if (uxtheme_dll)
	{
		xp_close();
		xp_open();
		_gtk_rc_reset_styles (gtk_settings_get_default());
	}
...
xp_open()
{
	...
	BOOL (FAR PASCAL *xp_IsAppThemed) (void);

	if(uxtheme_dll || notfound == 1)
		return;

	uxtheme_dll = LoadLibraryA("uxtheme.dll");
	if(!uxtheme_dll)
	{
		notfound = 1;
		return;
	...
	xp_IsAppThemed = (void *)GetProcAddress(uxtheme_dll, "IsAppThemed");
	if(!xp_IsAppThemed())
	{
	...
	xp_OpenThemeData = (void *)GetProcAddress(uxtheme_dll, "OpenThemeData");
	xp_DrawThemeBackground = (void *)GetProcAddress(uxtheme_dll, "DrawThemeBackground");
	xp_EnableThemeDialogTexture = (void *)GetProcAddress(uxtheme_dll, "EnableThemeDialogTexture");
	xp_CloseThemeData = (void *)GetProcAddress(uxtheme_dll, "CloseThemeData");
...
//	gdk_window_remove_filter (NULL, global_filter_func, NULL);
	...
	{
		if(open_themes[i])
			xp_CloseThemeData(open_themes[i]);
	}
...
static HTHEME
xp_get_theme(int type)
{
	LPCWSTR name = L"Scrollbar";
	int i = 0;
	...
	case XP_RADIO:
	case XP_RADIOP:
	case XP_BUTTON: name = L"Button"; i=1; break;
	case XP_LISTHEAD: name = L"Header"; i=3; break;
	case XP_COMBOBUTTON: name = L"ComboBox"; i=4; break;
	case XP_BODY:
	case XP_TAB: name = L"Tab"; i=5; break;
	case XP_VSCROLLBAR: break;
	case XP_HSCROLLBAR: break;
	case XP_ARROWUP: break;
	case XP_ARROWDOWN: break;
	case XP_ARROWLEFT: break;
	case XP_ARROWRIGHT: break;
	case XP_VGRIP: break;
	case XP_HGRIP: break;
	case XP_VTROUGH: break;
	case XP_HTROUGH: break;
	case XP_EDITTEXT: name = L"Edit"; i=2; break;
	default: return NULL;
	}

	if(!open_themes[i])
		open_themes[i] = xp_OpenThemeData(NULL, name);

	return open_themes[i];
}

static int
xp_draw(GdkWindow *win, int type, GtkStyle *style,
	int x, int y, int width, int height, int state_type)
{
	HTHEME theme;
	RECT rect;
	int xoff, yoff, state;
	...
	theme = xp_get_theme(type);
	if(!theme)
		return 0;

	xp_EnableThemeDialogTexture(GDK_WINDOW_HWND(win), ETDT_ENABLETAB);
...
/*	case XP_TAB:
		switch(state_type)
		{
		case GTK_STATE_PRELIGHT: state = TIS_HOT; break;
		case GTK_STATE_INSENSITIVE: state = TIS_DISABLED; break;
		case GTK_STATE_SELECTED:
		case GTK_STATE_ACTIVE: state = TIS_SELECTED; break;
		default: state = ETS_NORMAL;
		}
		break;*/
	...
	case XP_EDITTEXT:
		switch(state_type)
		{
		case GTK_STATE_PRELIGHT: state = ETS_FOCUSED; break;
		case GTK_STATE_INSENSITIVE: state = ETS_READONLY; break;
		case GTK_STATE_SELECTED:
	...
	case XP_VTROUGH:
		...
		{
		case GTK_STATE_SELECTED:
		case GTK_STATE_ACTIVE: state = SCRBS_PRESSED; break;
		case GTK_STATE_PRELIGHT: state = SCRBS_HOT; break;
		case GTK_STATE_INSENSITIVE: state = SCRBS_DISABLED; break;
		default: state = SCRBS_NORMAL;
		}
		break;
	...
	case XP_ARROWRIGHT:
		switch(state_type)
		{
		case GTK_STATE_ACTIVE: state = ABS_RIGHTPRESSED; break;
		case GTK_STATE_PRELIGHT: state = ABS_RIGHTHOT; break;
		case GTK_STATE_INSENSITIVE: state = ABS_RIGHTDISABLED; break;
		default: state = ABS_RIGHTNORMAL;
		}
		break;
	...
	case XP_ARROWLEFT:
		switch(state_type)
		{
		case GTK_STATE_ACTIVE: state = ABS_LEFTPRESSED; break;
		case GTK_STATE_PRELIGHT: state = ABS_LEFTHOT; break;
		case GTK_STATE_INSENSITIVE: state = ABS_LEFTDISABLED; break;
		default: state = ABS_LEFTNORMAL;
		}
		break;
	...
	case XP_ARROWDOWN:
		switch(state_type)
		{
		case GTK_STATE_ACTIVE: state = ABS_DOWNPRESSED; break;
		case GTK_STATE_PRELIGHT: state = ABS_DOWNHOT; break;
		case GTK_STATE_INSENSITIVE: state = ABS_DOWNDISABLED; break;
		default: state = ABS_DOWNNORMAL;
		}
		break;
	...
	case XP_ARROWUP:
		switch(state_type)
		{
		case GTK_STATE_ACTIVE: state = ABS_UPPRESSED; break;
		case GTK_STATE_PRELIGHT: state = ABS_UPHOT; break;
		case GTK_STATE_INSENSITIVE: state = ABS_UPDISABLED; break;
		default: state = ABS_UPNORMAL;
		}
		break;
	...
	case XP_CHECKBOX:
		switch(state_type)
		{
	//	case GTK_STATE_ACTIVE: state = CBS_UNCHECKEDPRESSED; break;
		case GTK_STATE_SELECTED: state = CBS_CHECKEDPRESSED; break;
		case GTK_STATE_PRELIGHT: state = CBS_UNCHECKEDHOT; break;
		case GTK_STATE_INSENSITIVE: state = CBS_UNCHECKEDDISABLED; break;
		default: state = CBS_UNCHECKEDNORMAL;
		}
		break;
	...
	case XP_CHECKBOXP:
		switch(state_type)
		{
	//	case GTK_STATE_ACTIVE: state = CBS_CHECKEDPRESSED; break;
		case GTK_STATE_SELECTED: state = CBS_CHECKEDPRESSED; break;
		case GTK_STATE_PRELIGHT: state = CBS_CHECKEDHOT; break;
		case GTK_STATE_INSENSITIVE: state = CBS_CHECKEDDISABLED; break;
		default: state = CBS_CHECKEDNORMAL;
		}
		break;
	...
	case XP_RADIO:
		switch(state_type)
		{
		case GTK_STATE_ACTIVE: state = RBS_UNCHECKEDPRESSED; break;
		case GTK_STATE_SELECTED: state = RBS_UNCHECKEDPRESSED; break;
		case GTK_STATE_PRELIGHT: state = RBS_UNCHECKEDHOT; break;
		case GTK_STATE_INSENSITIVE: state = RBS_UNCHECKEDDISABLED; break;
		default: state = RBS_UNCHECKEDNORMAL;
		}
		break;
	...
	case XP_RADIOP:
		switch(state_type)
		{
		case GTK_STATE_ACTIVE: state = RBS_CHECKEDPRESSED; break;
		case GTK_STATE_SELECTED: state = RBS_CHECKEDPRESSED; break;
		case GTK_STATE_PRELIGHT: state = RBS_CHECKEDHOT; break;
		case GTK_STATE_INSENSITIVE: state = RBS_CHECKEDDISABLED; break;
		default: state = RBS_CHECKEDNORMAL;
		}
		break;
	...
	default:
		switch(state_type)
		{
		case GTK_STATE_ACTIVE: state = PBS_PRESSED; break;
		case GTK_STATE_PRELIGHT: state = PBS_HOT; break;
		case GTK_STATE_INSENSITIVE: state = PBS_DISABLED; break;
		case 5: state = PBS_DEFAULTED; break;
		default: state = PBS_NORMAL;
		}
	}

	xp_DrawThemeBackground(theme, dc, xp_bg_types[type], state, &rect, 0);
...
static void
get_col (GdkColor *col, int sysc)
{
	col->pixel = GetSysColor (sysc);
	col->red = (GetRValue (col->pixel) << 8)   | GetRValue (col->pixel);
	col->green = (GetGValue (col->pixel) << 8) | GetGValue (col->pixel);
	col->blue = (GetBValue (col->pixel) << 8)  | GetBValue (col->pixel);
}
...
get_sys_colors (void)
{
	...
	get_col (&gtk_default_normal_fg, COLOR_WINDOWTEXT);
	get_col (&gtk_default_active_fg, COLOR_WINDOWTEXT);
	get_col (&gtk_default_prelight_fg, COLOR_HIGHLIGHTTEXT);
	get_col (&gtk_default_selected_fg, COLOR_HIGHLIGHTTEXT);
	get_col (&gtk_default_insensitive_fg, COLOR_GRAYTEXT);

	get_col (&gtk_default_normal_bg, COLOR_3DFACE);
	get_col (&gtk_default_active_bg, COLOR_SCROLLBAR);
	get_col (&gtk_default_prelight_bg, COLOR_HIGHLIGHT);
	get_col (&gtk_default_selected_bg, COLOR_HIGHLIGHT);
	get_col (&gtk_default_insensitive_bg, COLOR_3DFACE);

	get_col (&gtk_default_normal_base, COLOR_WINDOW);
	get_col (&gtk_default_active_base, COLOR_HIGHLIGHT);
	get_col (&gtk_default_prelight_base, COLOR_WINDOW);
	get_col (&gtk_default_selected_base, COLOR_HIGHLIGHT);
	get_col (&gtk_default_insensitive_base, COLOR_3DFACE);//INACTIVEBORDER);

	get_col (&gtk_default_normal_text, COLOR_WINDOWTEXT);
	get_col (&gtk_default_active_text, COLOR_HIGHLIGHTTEXT);
	get_col (&gtk_default_prelight_text, COLOR_HIGHLIGHTTEXT);
	get_col (&gtk_default_selected_text, COLOR_HIGHLIGHTTEXT);
	get_col (&gtk_default_insensitive_text, COLOR_GRAYTEXT);

	get_col (&gtk_default_dark, COLOR_3DSHADOW);
	get_col (&gtk_default_light, COLOR_3DHILIGHT);

	col = GetSysColor (COLOR_HIGHLIGHT);
	sprintf (rc_data + 46, "%02x%02x%02x",
				GetRValue (col), GetGValue (col), GetBValue (col));
	rc_data[46 + 6] = '"';

	col = GetSysColor (COLOR_HIGHLIGHTTEXT);
	sprintf (rc_data + 70, "%02x%02x%02x",
				GetRValue (col), GetGValue (col), GetBValue (col));
	rc_data[70 + 6] = '"';

	col = GetSysColor (COLOR_MENUTEXT);
	sprintf (rc_data + 92, "%02x%02x%02x",
				GetRValue (col), GetGValue (col), GetBValue (col));
	rc_data[92 + 6] = '"';

	col = GetSysColor (COLOR_MENU);
	sprintf (rc_data + 114, "%02x%02x%02x",
				GetRValue (col), GetGValue (col), GetBValue (col));
	rc_data[114 + 6] = '"';
}
...
gtk_style_init (GtkStyle *style)
{
  gint i;
  static int done = 0;
  static int syscolors = 0;
  GtkSettings *settings = gtk_settings_get_default ();
  
  if (settings)
    _gtk_style_init_for_settings (style, settings);
  else
    style->font_desc = pango_font_description_from_string ("Sans 8");
  ...
  style->base[GTK_STATE_NORMAL] = gtk_default_normal_base;
  style->base[GTK_STATE_ACTIVE] = gtk_default_active_base;
  style->base[GTK_STATE_PRELIGHT] = gtk_default_prelight_base;
  style->base[GTK_STATE_SELECTED] = gtk_default_selected_base;
  style->base[GTK_STATE_INSENSITIVE] = gtk_default_insensitive_base;

  style->text[GTK_STATE_NORMAL] = gtk_default_normal_text;
  style->text[GTK_STATE_ACTIVE] = gtk_default_active_text;
  style->text[GTK_STATE_PRELIGHT] = gtk_default_prelight_text;
  style->text[GTK_STATE_SELECTED] = gtk_default_selected_text;
  style->text[GTK_STATE_INSENSITIVE] = gtk_default_insensitive_text;
...
gtk_style_real_realize (GtkStyle *style)
{
  ...
  for (i = 0; i < 5; i++)
    {
	style->light[i] = gtk_default_light;
	style->dark[i] = gtk_default_dark;
			  ...
			  
{
  ...
  bitmap = indicator_parts[part].bmap;
  if (bitmap)
    return bitmap;
  ...
  indicator_parts[part].bmap = bitmap;
                         ...
                         gint           height)
{
  ...
  g_return_if_fail (window != NULL);
	...
	if(detail && !strcmp(detail, "entry"))
	{
		...
		if(xp_draw(window, XP_EDITTEXT, style, x, y, width, height, state_type))
			return;
	}
#endif
  ...
  switch (shadow_type)
    ...
    case GTK_SHADOW_IN:
      ...
      if (style->ythickness > 1)
        gdk_draw_line (window, gc1,//style->black_gc,
...
#define IS_MENU_ITEM(detail) (detail && detail[0] == 'm' && detail[1] == 'i' && detail[2] == 't' && detail[3] == 'e' && detail[4] == 'm' && detail[5] == 0)
...
#ifdef USE_XP
  if(uxtheme_dll && GTK_IS_BUTTON (widget->parent))	/* combo */
  {
		if(widget->parent->parent)
		{
			if(GTK_IS_COMBO (widget->parent->parent))
				return;
		}
  }

//  if (detail && strcmp (detail, "mitem") != 0)
  if (!(IS_MENU_ITEM (detail)))
  {
  box_x = x;
  box_y = y;
  box_width = width;
  ...
  {
  case GTK_ARROW_UP:
 	if(xp_draw(window, XP_ARROWUP, style, box_x, box_y, box_width, box_height, state))
		return;
	break;

  case GTK_ARROW_DOWN:
 	if(xp_draw(window, XP_ARROWDOWN, style, box_x, box_y, box_width, box_height, state))
		return;
	break;

  case GTK_ARROW_LEFT:
 	if(xp_draw(window, XP_ARROWLEFT, style, box_x, box_y, box_width, box_height, state))
		return;
	break;

  case GTK_ARROW_RIGHT:
 	if(xp_draw(window, XP_ARROWRIGHT, style, box_x, box_y, box_width, box_height, state))
		return;
	break;
...
//  if (detail && strcmp (detail, "mitem") == 0)
  ...
  {
    /* dont make arrows white while cursor is on it */
    if (!uxtheme_dll && state == GTK_STATE_PRELIGHT)
		      ...
		      gint           height)
{
  ...
  if(GTK_IS_COMBO (widget->parent))
  {
	if(xp_draw(window, XP_EDITTEXT, style, x, y, width, height, 0))
	{
 		xp_draw(window, XP_COMBOBUTTON, style, x+1, y+1, width-2, height-2, state_type);
		return;
	}
  }
  ...
  {
	void *stylehint = g_object_get_data (G_OBJECT (widget), "xchat-color");

	if(stylehint == (void *)1) {}
	else
	{
		if(state_type == GTK_STATE_NORMAL && !strcmp(detail, "buttondefault"))
			state_type = 5;

		if(uxtheme_dll && stylehint == (void *)2)
		{
			if (state_type == GTK_STATE_ACTIVE)
				width -= 1;
			xp_draw(window, XP_TAB, style, x, y, width, height, state_type);
			y += height;
			y -= 1;
			width -= 2;
			gdk_draw_line (window, style->mid_gc[0], x + 1, y, x + width, y);
			return;
		} else if(GTK_IS_TREE_VIEW (widget->parent) || GTK_IS_CLIST (widget->parent))
		{
			if(xp_draw(window, XP_LISTHEAD, style, x, y, width, height, state_type))
				return;
		} else
		{
			if(xp_draw(window, XP_BUTTON, style, x, y, width, height, state_type))
				return;
		}
	}
	  if(state_type == GTK_STATE_PRELIGHT || state_type == 5)
  ...
  if(detail && !strcmp(detail, "trough"))
  {
	...
	{
		if(xp_draw(window, XP_VTROUGH, style, x, y, width, height, 0))
			return;
	} else
	{
		if(xp_draw(window, XP_HTROUGH, style, x, y, width, height, 0))
			return;
	}
  }
#endif
...
#ifdef USE_XP
  if(detail && !strcmp(detail, "menubar"))
  {
	if(xp_draw(window, XP_BODY, style, x, y, width, height, 0))
		return;
  }
#endif
 ...
 /* if (detail && !strcmp(detail, "menu"))
  {
    gdk_draw_rectangle (window, style->mid_gc[0], FALSE,
			x, y, width, height);
    return;
  }*/

/*  if (detail && !strcmp(detail, "mitem") && state_type == GTK_STATE_PRELIGHT)
  {
    if (!GTK_IS_MENU_BAR (widget->parent))
	{
		x += 8;
...
#if 0//def USE_XP
	if(detail && !strcmp(detail, "entry_bg"))
	{
		if(GTK_WIDGET_HAS_FOCUS (widget))
			state_type = GTK_STATE_PRELIGHT;
		if(xp_draw(window, XP_EDITTEXT, style, x, y, width, height, state_type))
          ...
          else if (!strcmp ("entry_bg", detail))
{
	  ...
	  else if (!strcmp ("cell-odd-ruled", detail))
	    {
	      GdkColor *color;
			...
			gint           height)
{
  if (detail && strcmp (detail, "cellcheck") == 0)
	...
	if(shadow_type == GTK_SHADOW_IN)
	{
	  if(xp_draw(window, XP_CHECKBOXP, style, x, y, width, height, state_type))
		return;
	} else
	{
	  if(xp_draw(window, XP_CHECKBOX, style, x, y, width, height, state_type))
		return;
	}
#endif
			 ...
			 gint           height)
{
  if (detail && strcmp (detail, "cellradio") == 0)
  ...
  else
    {
      ...
      GdkGC *text_gc;
      GdkGC *aa_gc;
      gboolean is_option = FALSE;

	if (detail && !strcmp (detail, "option"))
		is_option = TRUE;

#ifdef USE_XP
	if (!is_option)	/* !Menu item */
	{
	if (shadow_type == GTK_SHADOW_IN)
	{
		if(xp_draw(window, XP_RADIOP, style, x, y, width, height, state_type))
			return;
	} else
	{
		if(xp_draw(window, XP_RADIO, style, x, y, width, height, state_type))
			return;
	}
	}
#endif
      ...
      else
	{
      ...
      if (shadow_type == GTK_SHADOW_IN)
	{
		/* no white bullets for prelight radio buttons on '98 */
		if (!is_option && state_type == GTK_STATE_PRELIGHT)
			text_gc = style->fg_gc[GTK_STATE_ACTIVE];
...
#define ARROW_SPACE 4
  ...
  {
	if(xp_draw(window, XP_COMBOBUTTON, style, x-5, y-3, width+10, height+6, state_type))
		return;
  }
#endif
                         ...
                         GtkOrientation orientation)
{
  ...
  {
	  if(xp_draw(window, XP_HSCROLLBAR, style, x, y, width, height, state_type))
	  {
		xp_draw(window, XP_HGRIP, style, x, y, width, height, 0);
		return;
	  }
  } else
  {
  	if(xp_draw(window, XP_VSCROLLBAR, style, x, y, width, height, state_type))
 	 {
		xp_draw(window, XP_VGRIP, style, x, y, width, height, 0);
		return;
 	 }
  }
#endif
  ...
  if (detail &&
    ...
    {
      if (orientation != GTK_ORIENTATION_HORIZONTAL && height > 9)
	{	/* scrollbar grip for '98 */
        gtk_default_draw_hline (style, window, state_type, area, widget, NULL, 
                         x + style->xthickness, 
                         x + width - style->xthickness - 1, (y + height / 2) - 2);
        gtk_default_draw_hline (style, window, state_type, area, widget, NULL,
                         x + style->xthickness, 
                         x + width - style->xthickness - 1, (y + height / 2));
        gtk_default_draw_hline (style, window, state_type, area, widget, NULL, 
...
#define DETAIL(a) (a && !strcmp(detail,a))
	...
	if (DETAIL("dockitem") ||
	    (widget
	     && strcmp(g_type_name(G_TYPE_FROM_INSTANCE(widget)),
		       "PanelAppletFrame") == 0)) {
		/* Fix orientation bug */
	...
	if ((DETAIL("handlebox") && widget && GTK_IS_HANDLE_BOX(widget))
		...
		gdk_draw_line(window,
			      (style)->light_gc[2],
			      x, y + height - 1, x + width,
			      y + height - 1);

		gdk_draw_line(window,
			      (style)->light_gc[2],
			      x + width - 1, y + 2, x + width - 1,
			      y + height - 3);
		gdk_draw_line(window,
			      (style)->dark_gc[0],
			      x + width, y + 2, x + width, y + height - 3);
		...
		if (orientation == GTK_ORIENTATION_HORIZONTAL) {
			py = y + height / 2 - 1;
			for (px = x + width / 2 - 15;
			     px <= x + width / 2 + 15; px += 5) {
				gdk_draw_point(window, light_gc, px, py);
				gdk_draw_point(window, light_gc, px + 1,
					       py);
				gdk_draw_point(window, light_gc, px,
					       py + 1);
				gdk_draw_point(window, dark_gc, px + 1,
					       py + 2);
				gdk_draw_point(window, dark_gc, px + 2,
					       py + 1);
				gdk_draw_point(window, dark_gc, px + 2,
					       py + 2);
			}
			...
			px = x + width / 2 - 1;
			for (py = y + height / 2 - 15;
			     py <= y + height / 2 + 15; py += 5) {
				gdk_draw_point(window, light_gc, px, py);
				gdk_draw_point(window, light_gc, px + 1,
					       py);
				gdk_draw_point(window, light_gc, px,
		...
		gtk_default_draw_box(style, window, state_type, shadow_type, area,
	...
	if (orientation == GTK_ORIENTATION_VERTICAL) {
		if (height >= 14 && width >= 8) {
			...
			gdk_draw_line(window, light_gc, x + 3, gy + 5,
				      x + 3 + glength, gy + 5);
			gdk_draw_line(window, dark_gc, x + 2, gy + 4,
				      x + 2 + glength, gy + 4);

			gdk_draw_line(window, light_gc, x + 3, gy + 8,
				      x + 3 + glength, gy + 8);
			gdk_draw_line(window, dark_gc, x + 2, gy + 7,
				      x + 2 + glength, gy + 7);

			gdk_draw_line(window, light_gc, x + 3, gy + 11,
				      x + 3 + glength, gy + 11);
			gdk_draw_line(window, dark_gc, x + 2, gy + 10,
				      x + 2 + glength, gy + 10);
		}
	} else {
		if (width > 14 && height >= 8) {
			...
			gdk_draw_line(window, light_gc, gx + 5, y + 3,
				      gx + 5, y + 3 + glength);
			gdk_draw_line(window, dark_gc, gx + 4, y + 2,
				      gx + 4, y + 2 + glength);

			gdk_draw_line(window, light_gc, gx + 8, y + 3,
				      gx + 8, y + 3 + glength);
			gdk_draw_line(window, dark_gc, gx + 7, y + 2,
				      gx + 7, y + 2 + glength);

			gdk_draw_line(window, light_gc, gx + 11, y + 3,
				      gx + 11, y + 3 + glength);
			gdk_draw_line(window, dark_gc, gx + 10, y + 2,
				      gx + 10, y + 2 + glength);
		}
	}
