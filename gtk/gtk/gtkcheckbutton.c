...
{
  return g_object_new (GTK_TYPE_CHECK_BUTTON, "label", label, "use-underline", TRUE, NULL);
}
  ...
  if (GTK_WIDGET_DRAWABLE (check_button))
      ...
      if (GTK_WIDGET_STATE (toggle_button) == GTK_STATE_PRELIGHT)
	{
	  ...
	  if (gdk_rectangle_intersect (area, &restrict_area, &new_area))
	    {
	      gtk_paint_flat_box (widget->style, widget->window, GTK_STATE_ACTIVE,
      ...
      gtk_paint_check (widget->style, widget->window,
		       (uxtheme_dll) ? GTK_WIDGET_STATE (toggle_button) : state_type, shadow_type,
		       area, widget, "checkbutton",
