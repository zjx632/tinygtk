...
gint         gtk_entry_find_position            (GtkEntry       *entry,
							gint            x);
...
gtk_entry_init (GtkEntry *entry)
{
  ...
  entry->force_new_layout = FALSE;
  entry->error_list = NULL;
...
gtk_entry_finalize (GObject *object)
{
  ...
  gtk_entry_clear_error_range (entry);
...
gtk_entry_draw_frame (GtkWidget *widget)
{
  ...
  if (!uxtheme_dll && GTK_WIDGET_HAS_FOCUS (widget) && !interior_focus)
    {
      x += focus_width;
      y += focus_width;
      width -= 2 * focus_width;
...
void
gtk_entry_clear_error_range (GtkEntry *entry)
{
	while(entry->error_list)
	{
		free (entry->error_list->data);
		entry->error_list = g_slist_remove(entry->error_list, entry->error_list->data);
	}

	entry->force_new_layout = TRUE;
}

void
gtk_entry_add_error_range (GtkEntry *entry, gint start, gint end)
{
	e_range *e = malloc(sizeof(e_range));

	e->start = start;
	e->end = end;

	entry->error_list = g_slist_prepend(entry->error_list, e);
			 ...
			 gboolean  include_preedit)
{
  ...
  pango_layout_set_single_paragraph_mode (layout, TRUE);
	...
	while(list)
	{
		...
		attr->start_index = e->start;
		attr->end_index = e->end;
                         ...
                         gboolean  include_preedit)
{
  if ((entry->preedit_length > 0 &&
      !include_preedit != !entry->cache_includes_preedit) || entry->force_new_layout)
	{
    entry->force_new_layout = FALSE;
    gtk_entry_reset_layout (entry);
	}
...
static void
AA_activate_cb (GtkWidget *menu_item,
             gpointer   data)
{
...
static void
AA_gtk_text_util_append_special_char_menuitems (GtkMenuShell              *menushell,
                                              GtkTextUtilCharChosenFunc  func,
                                              gpointer                   data)
{
  int i;
			...
			gpointer          user_data)
{
  ...
  if (GTK_WIDGET_REALIZED (entry))
	...
	gtk_widget_destroy (entry->popup_menu);
      ...
      AA_gtk_text_util_append_special_char_menuitems (GTK_MENU_SHELL (submenu),
                                                    unichar_chosen_func,
                                                    entry);
      if (!entry->editable)
