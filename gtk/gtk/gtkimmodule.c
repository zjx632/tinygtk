...
void 
cy_im_module_list (const GtkIMContextInfo ***contexts,
		guint                      *n_contexts);
void 
cy_im_module_exit (void);
void
cy_im_module_init (GTypeModule *module);

GtkIMContext *
cy_im_module_create (const gchar *context_id);
...
gtk_im_module_load (GTypeModule *module)
{
  ...
  im_module->list = cy_im_module_list;
  im_module->init = cy_im_module_init;
  im_module->exit = cy_im_module_exit;
  im_module->create = cy_im_module_create;
...
gtk_im_module_init ()
{
  ...
  info = g_new0 (GtkIMContextInfo, 1);
  info->context_id = g_strdup ("cyrillic_translit");
  info->context_name = g_strdup ("Cyrillic (Transliterated)");
  info->domain = g_strdup ("gtk+");
  info->domain_dirname = g_strdup ("./locale");
  info->default_locales = g_strdup ("");

  module = g_object_new (GTK_TYPE_IM_MODULE, NULL);
  module->path = g_strdup ("./");
  module->library = NULL;
//  g_type_module_set_name (G_TYPE_MODULE (module), module->path);
  ((GTypeModule*)module)->name = g_strdup (module->path);

  add_module (module, g_slist_prepend (NULL, info));

#if 0
  info = g_new0 (GtkIMContextInfo, 1);
  info->context_id = g_strdup ("ime");
  info->context_name = g_strdup ("Windows IME");
  info->domain = g_strdup ("gtk+");
  info->domain_dirname = g_strdup ("./locale");
  info->default_locales = g_strdup ("ja:ko:zh");
