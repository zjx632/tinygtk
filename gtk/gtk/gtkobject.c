			 ...
			 GParamSpec   *pspec)
{
  switch (property_id)
    {
    case PROP_USER_DATA:
      g_object_set_data (G_OBJECT (object), "user-data", g_value_get_pointer (value));
      break;
    default:
			 ...
			 GParamSpec  *pspec)
{
  switch (property_id)
    {
    case PROP_USER_DATA:
      g_value_set_pointer (value, g_object_get_data (G_OBJECT (object), "user-data"));
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, property_id, pspec);
      break;
			  ...
			  gpointer   data)
{
  ...
  if (!quark_user_data)
    quark_user_data = g_quark_from_static_string ("user-data");
