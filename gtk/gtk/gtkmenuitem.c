...
gtk_menu_item_new (void)
{
  ...
  i = g_object_new (GTK_TYPE_MENU_ITEM, NULL);
  gtk_widget_set_double_buffered (i, FALSE);
  ...
  if (bin->child && GTK_WIDGET_VISIBLE (bin->child))
    {
      ...
      requisition->height += (child_requisition.height + 2);
...
if (state_type != GTK_STATE_PRELIGHT)
{
      ...
      else if (!GTK_BIN (menu_item)->child)
	{
	  gtk_paint_hline (widget->style, widget->window, GTK_STATE_NORMAL,
			   area, widget, "mitem",
			   widget->allocation.x+2, widget->allocation.width-3,
			   widget->allocation.y+3);
	}
    }
}
  ...
  if (menu_item->submenu)
    {
      ...
      if (menu_item->timer)
	g_timeout_remove (menu_item->timer);
