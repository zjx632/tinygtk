...
/* Prevent button 2 MOUSE_UP / MOUSE_DOWN events while using mouse wheel */
static guint32 last_mouse_wheel_time = 0;
#define GDK_WIN32_NO_MOUSE_WHEEL_CLICK_DELAY 200
		 ...
		 GdkEvent  **event)
{
  ...
  else if (!(((GdkWindowObject *) window)->event_mask & GDK_EXPOSURE_MASK))
    {
      GDK_NOTE (EVENTS, g_print (" (ignored)"));
    }

  else if ((paintstruct.rcPaint.right == paintstruct.rcPaint.left) ||
  ...
  /* Clean up */
  GDI_CALL(EndPaint, (msg->hwnd, &paintstruct));
  GDI_CALL(DeleteObject, (hrgn));
}
...
#define return GOTO_DONE_INSTEAD
  ...
  if (msg->message == msh_mousewheel)
    {
      ...
      last_mouse_wheel_time = GetTickCount();
  ...
  switch (msg->message)
    ...
    case WM_MBUTTONDOWN:
      if (GetTickCount() < last_mouse_wheel_time + GDK_WIN32_NO_MOUSE_WHEEL_CLICK_DELAY) {
        return_val = FALSE;
        goto done;
      }
      button = 2;
