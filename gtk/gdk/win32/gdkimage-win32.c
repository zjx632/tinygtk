		      ...
		      gint       h)
{
  ...
  pixmap = gdk_pixmap_new_internal (NULL, w, h, 1, TRUE);

  if (pixmap == NULL)
    return NULL;

  g_assert (GDK_PIXMAP_IMPL_WIN32 (GDK_PIXMAP_OBJECT (pixmap)->impl)->bits);
			  ...
			  gint          depth)
{
  ...
  pixmap = gdk_pixmap_new_internal (NULL, width, height, depth, TRUE);
