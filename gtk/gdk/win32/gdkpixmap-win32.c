...
static HBITMAP
gdk_create_compatible_bitmap (HDC  hdc,
                              gint width,
                              gint height)
{
  HBITMAP hbitmap;
  ...
  if (hbitmap == NULL)
    WIN32_GDI_FAILED("CreateCompatibleBitmap");
  
  return hbitmap;
}

static HBITMAP
gdk_create_di_bitmap (HDC hdc,
                             gint         width,
                              gint         height,
			      gint         depth,
			      ...
			      guchar **bits)
{
  ...
  if (depth == 1)
    ...
    {
      if (depth <= 8 && colormap != NULL)
	{
	  GdkColormapPrivateWin32 *cmapp = GDK_WIN32_COLORMAP_DATA (colormap);
	  gint k;
...
static int dibs_enabled = 0;

void
gdk_pixmap_enable_dibs (void)
{
  dibs_enabled = 1;
}

GdkPixmap*
gdk_pixmap_new_internal (GdkDrawable *drawable,
			 gint         width,
			 gint         height,
			 gint         depth,
			 gboolean     force_dib)
...
/*  g_return_val_if_fail (drawable == NULL || GDK_IS_DRAWABLE (drawable), NULL);
  ...
  pixmap_impl->is_compat_bitmap = (!force_dib && depth == window_depth && depth != 1);
  if (pixmap_impl->is_compat_bitmap)
    hbitmap = gdk_create_compatible_bitmap (hdc, width, height);
  else
    hbitmap = gdk_create_di_bitmap (hdc, width, height, depth, cmap, &bits);
  if (hbitmap == NULL)
    goto failure;
...
gdk_pixmap_new (GdkDrawable *drawable,
		...
		gint         depth)
{
  return gdk_pixmap_new_internal (drawable, width, height, depth, FALSE);
}
