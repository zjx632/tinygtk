		...
		int             end_x)
{
  ...
  switch (uline)
	...
	if (!red)
	{
		...
		red = gdk_gc_new (drawable);
		gdk_colormap_alloc_color (gdk_drawable_get_colormap (drawable), &col, TRUE, TRUE);
		gdk_gc_set_foreground (red, &col);
	}
      ...
      if (!shape_set)
        {
          ...
          if (embossed)
            {
              ...
              gdk_draw_qlyphs (drawable, white_gc, run->item->analysis.font,
                               gx + 1,
                               gy + 1,
                               run->glyphs);
          ...
          gdk_draw_qlyphs (drawable, fg_gc, run->item->analysis.font,
                           gx, gy,
                           run->glyphs);
        }
